...
$tajuk = $_GET["soalan"];
$res=mysqli_query($conn,"select * from jawapanpelajar where Tajuk = '$tajuk'");
...
<?php
        $res=mysqli_query($conn,"select * from tugasan where Tajuk = '$tajuk'");
while($row=mysqli_fetch_array($res)){
    $subjek = $row["Subjek"];
    $tajuk = $row["Tajuk"];
    $tarikh = $row["tarikh"];
...
<h3 style="color: green;">Subjek: <?php echo $subjek; ?></h3>
            ...
            <hr class="w3-clear">
            <h3 class="w3-center">Tajuk: <?php echo $tajuk; ?></h3>
...
//Paparkan senarai jawapan pelajar dalam table
$count = 0;
    $res = mysqli_query($conn,"select * from jawapanpelajar where Tajuk='$tajuk' ORDER BY id DESC");
    while($row=mysqli_fetch_array($res)){
        $count = $count + 1;
        ?>
    <tr>
        ...
        <td><a href="jawapanPelajar_lihat_muatTurun.php?muatTurun=<?php echo $row["id"]; ?>"><button>Muat Turun Fail Pelajar</button></td>
    </tr>
